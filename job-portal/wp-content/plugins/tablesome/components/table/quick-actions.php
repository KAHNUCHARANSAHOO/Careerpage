...
if ( !class_exists( '\\Tablesome\\Components\\Table\\Quick_Actions' ) ) {
    class Quick_Actions
    {
        ...
        private function get_empty_table_action_url( $table )
        {
            ...
            $title = __( 'Empty Table', 'tablesome' );
            $link_text = __( 'Empty Table', 'tablesome' );
            return '<a class="tablesome__table-action--emptyTable" href="javascript:void(0);" data-url="' . $url . '" title="' . $title . '" rel="permalink" style="color:#b32d2e;">' . $link_text . '</a>';
        }
        ...
        public function show_notices()
        {
            ...
            $status_content = array(
                ...
                'FAILED_TO_CREATE_EMAIL_LOGS_TABLE' => array(
                'class'   => 'notice-warning',
                'message' => __( 'Failed to create email logs table, Please try again.', 'tablesome' ),
            ),
            );
