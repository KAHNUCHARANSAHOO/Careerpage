    ...
    class Actions
    {
        ...
        public function get_email_logs_trigger_table_info()
        {
            ...
            if ( $email_logs_trigger_publish_table_id ) {
                return [
                    'url'    => admin_url( 'edit.php?post_type=' . TABLESOME_CPT . '&action=edit&page=tablesome_admin_page&post=' . $email_logs_trigger_publish_table_id ),
                    'status' => 'table_exists',
                ];
            } else {
                
                if ( $email_logs_trigger_draft_table_id ) {
                    $url = admin_url( 'edit.php?post_type=' . TABLESOME_CPT . '&action=edit&page=tablesome_admin_page&post=' . $email_logs_trigger_draft_table_id );
                    $url = add_query_arg( array(
                        'link_action' => 'EMAIL_LOGS',
                        'status'      => 'PUBLISH_EMAIL_LOGS_TABLE',
                    ), $url );
                    return [
                        'url'    => $url,
        ...
        public function create_new_email_logs_trigger_table()
        {
            ...
            $table_id = ( new \Tablesome\Includes\Modules\Onboarding() )->create_email_logs_table();
            
            if ( $table_id ) {
                // redirect to edit table page
                $url = admin_url( 'edit.php?post_type=' . TABLESOME_CPT . '&action=edit&page=tablesome_admin_page&link_action=EMAIL_LOGS&status=PUBLISH_EMAIL_LOGS_TABLE&post=' . $table_id );
            } else {
                // redirect to create new table page
                $url = admin_url( 'edit.php?post_type=' . TABLESOME_CPT . '&page=tablesome_admin_page&link_action=EMAIL_LOGS&status=FAILED_TO_CREATE_EMAIL_LOGS_TABLE' );
            }
