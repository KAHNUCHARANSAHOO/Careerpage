    ...
    abstract class Trigger
    {
        ...
        public function run_triggers($trigger_class, $trigger_source_data)
        {
            ...
            foreach ($trigger_instances as $trigger_instance) {
                ...
                // Generate a unique ID for each trigger instance
                $trigger_instance_id = md5(uniqid(rand(), true));
                $workflow_data[$trigger_instance_id] = $this->get_workflow_data($trigger_source_data);

                $trigger_instance['_trigger_instance_id'] = $trigger_instance_id;
